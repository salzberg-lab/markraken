...
size_t NcbiTaxonomy::loadNodes(const std::string &nodesFile) {
    std::cout << "Loading nodes file ...";
    std::ifstream ss(nodesFile);
    if (ss.fail()) {
        std::cout << "File " << nodesFile << " not found!\n";
//        EXIT(EXIT_FAILURE);
        exit(EXIT_FAILURE);
    }
    ...
    for (std::vector<TaxonNode>::iterator it = taxonNodes.begin(); it != taxonNodes.end(); ++it) {
        if (!nodeExists(it->parentTaxId)) {
            std::cout << "Inconsistent nodes.dmp taxonomy file! Cannot find parent taxon with ID " << it->parentTaxId << "!\n";
//            EXIT(EXIT_FAILURE);
            exit(EXIT_FAILURE);
        }
    }

    std::cout << " Done, got " << taxonNodes.size() << " nodes\n";
...
void NcbiTaxonomy::loadNames(const std::string &namesFile) {
    std::cout << "Loading names file ...\n";
    std::ifstream ss(namesFile);
    if (ss.fail()) {
        std::cout << "File " << namesFile << " not found!\n";
//        EXIT(EXIT_FAILURE);
        exit(EXIT_FAILURE);
    }
    ...
    while (std::getline(ss, line)) {
        ...
        if (!nodeExists(entry.first)) {
            std::cout << "loadNames: Taxon " << entry.first << " not present in nodes file!\n";
//            EXIT(EXIT_FAILURE);
            exit(EXIT_FAILURE);
        }
...
bool NcbiTaxonomy::IsAncestor(TaxID ancestor, TaxID child) {
    ...
    if (!nodeExists(child)) {
        std::cout << "No node for taxID " << child << ".\n";
        return false;
    } 

    if (!nodeExists(ancestor)) {
        std::cout << "No node for taxID " << ancestor << ".\n";
        return false;
    }
...
TaxonNode const * NcbiTaxonomy::LCA(const std::vector<TaxID>& taxa) const {
    ...
    while (it != taxa.end() && !nodeExists(*it)) {
        std::cout << "No node for taxID " << *it << ", ignoring it.\n";
        ++it;
    }
...
size_t NcbiTaxonomy::loadMerged(const std::string &mergedFile) {
    std::cout << "Loading merged file ...";
    std::ifstream ss(mergedFile);
    if (ss.fail()) {
        std::cout << "File " << mergedFile << " not found!\n";
//        EXIT(EXIT_FAILURE);
        exit(EXIT_FAILURE);
    }
...
//    std::cout << "Loading NCBI taxonomy\n";
...
//    } else {
//        std::cout << "names.dmp, nodes.dmp, merged.dmp from NCBI taxdump could not be found!\n";
////        EXIT(EXIT_FAILURE);
//        exit(EXIT_FAILURE);
//    }
